...
function* callMarkAsRead(action: {[Key: string]: any}) {
    try {
        ...
        if(!result || !data){
            yield put({ type: MARK_AS_READ_FAILED, payload: data });
        }

        if(status === 500){
            yield put({ type: MARK_AS_READ_INTERNAL_SERVER_ERROR, payload: data });
        }
    } catch(err){
        yield put({ type: MARK_AS_READ_FAILED, payload: err || err.message});
    }
}
