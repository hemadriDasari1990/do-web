...
const NoteList = (props: any) => {
    const { sectionId, noteList, setShowNote, updateTotalNotes } = props;
    const { adornmentStyle, startAdornmentStyle, pastTimeStyle, textfieldStyle, iconStyle } = useStyles();
            ...
            <Box p={1}>
                ...
                {Array.isArray(notes) && notes.map((note: {[Key: string]: any}) => (
                    <Grid item key={note._id} xl={12} lg={12} md={12} sm={12} xs={12}>
                                        ...
                                        <InputAdornment position="start">
                                            <Box display="flex" mb={3} justifyContent="space-between" className={adornmentStyle}>
                                                ...
                                                <Box mt={0.5}>
                                                    <Tooltip title="Delete Note">
                                                        <Zoom in={true} timeout={1500}>
                                                            <IconButton className={iconStyle} size="small" onClick={(event: React.MouseEvent<HTMLButtonElement>) => handleReactionClick(event)}>
                        ...
                        <ReactionPopover anchorEl={anchorEl} note={note} handlePopoverClose={handlePopoverClose} />
                    </Grid>
                ))}
